...
from src.utils import get_numerical_columns, get_categorical_columns, argmax, get_model_performance
...
if __name__=='__main__':
    ...
    instantiate_model = partial(get_lgbm_classifier, numerical_columns=numerical_columns, categorical_columns=categorical_columns)
    ...
    best_10_percent_models = [*map(lambda trial: instantiate_model(trial).fit(X_train, y_train), best_10_percent_trials)]

    get_performance = partial(get_model_performance, X=X_test, y=y_test, function=brier_score_loss)
    best_model = argmax(best_10_percent_models, get_performance)
