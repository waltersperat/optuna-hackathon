...
def suggest_imputer_params(trial : Trial, categorical : bool=False) -> dict[str, Any]:
    if not categorical:
        ...
        strategy = trial.suggest_categorical('strategy', ['most_frequent', 'constant'])
...
def suggest_ohe_params(trial : Trial) -> dict[str, Any]:
    params = {
        ...
        'max_categories': trial.suggest_float('max_categories', 5, 20),
    }
    return params
...
def suggest_lgbm_params(trial : Trial) -> dict[str, Any]:
    params = {
        ...
        'learning_rate': trial.suggest_float('learning_rate', 0.05, 1e-6),
        'feature_fraction': trial.suggest_float('feature_fraction', 0.1, 1),
        'bagging_fraction': trial.suggest_float('bagging_fraction', 0.1, 1),
        'reg_alpha': trial.suggest_float('reg_alpha', 0, 20),
        'reg_lambda': trial.suggest_float('reg_lambda', 0, 20),
        'reg_lambda': trial.suggest_float('reg_lambda', 0, 20),
        'min_child_samples': trial.suggest_float('min_child_samples', 5, 200),
        'random_state': 42
    }
    return params


def get_model(trial : Trial, numerical_columns : list[str], categorical_columns : list[str], suggest_function : Callable[[Trial], LGBMClassifier]=suggest_lgbm_params) -> Pipeline:
    processor = get_processor(trial, numerical_columns, categorical_columns)
    model = Pipeline([
        ('processor', processor),
        ('model', suggest_function(trial))
    ])
    return model
